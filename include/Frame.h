...
public:
    ...
    Frame(const cv::Mat &imLeft, const cv::Mat &imRight, const double &timeStamp, ORBextractormask* extractorLeft, ORBextractormask* extractorRight, ORBVocabulary* voc, cv::Mat &K, cv::Mat &distCoef, const float &bf, const float &thDepth, const cv::Mat &imMask, const int extractTwice);
    ...
    void ExtractORB_mask(int leftrightflag, const cv::Mat &im, const cv::Mat &imMask, const int extractTwice);//runqiu:add mask to 
...
public:
    ...
    std::vector<cv::KeyPoint> mvKeys_all, mvKeysRight_all;;//runqiu:用于存放左图过滤前的所有点
    ...
    cv::Mat mDescriptors_all, mDescriptorsRight_all;//runqiu:用于存放左图过滤前的所有点
