...
namespace ORB_SLAM2 { namespace cuda {
  ...
  public:
    ...
    void detect(InputArray, std::vector<KeyPoint>&, InputArray, float, float, int);

    void detectAsync(InputArray);
    void joinDetectAsync(std::vector<KeyPoint>&, InputArray, float, float, int);
  };
  ...
  class IC_Angle {
    ...
    KeyPoint * keypoints;
    cudaStream_t stream;
    Stream _cvStream;//runqiu: a queue for parrallel processing by cuda
  public:
